...
class PackageDetailDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, api_client: ApiClient, subscription: dict, on_refresh=None):
        ...
        pkg_name = subscription.get('package', {}).get('name', 'Paket')
        ...
        status_map = {
            'active': ('🟢 Aktif', '#2CC985'),
            'expired': ('🔴 Süresi Dolmuş', '#E04F5F'),
            'cancelled': ('⚫ İptal Edildi', '#888888')
        }
        status_text, status_color = status_map.get(status, ('⚪ Bilinmiyor', '#888888'))
        ...
        # Date Information
        self.create_info_section(scroll_frame, "📅 Tarih Bilgileri")
        
        start_date = self.format_date(subscription.get('start_date', ''))
        end_date = self.format_date(subscription.get('end_date', ''))
        
        self.create_info_row(scroll_frame, "Başlangıç", start_date)
        self.create_info_row(scroll_frame, "Bitiş", end_date)
        
        # Session Information
        self.create_info_section(scroll_frame, "🎯 Ders Kullanımı", top_padding=20)
        ...
        if access_type == 'TIME_BASED':
            ...
            ctk.CTkLabel(infinity_frame, text="♾️ Zaman Bazlı Erişim", 
                        font=("Roboto", 16, "bold"), 
                        text_color="white").pack(pady=(12, 5))
            ctk.CTkLabel(infinity_frame, text=f"Paket süresi boyunca sınırsız ders ({used} giriş yapıldı)", 
                        font=("Roboto", 12), 
            ...
            self.create_stat_box(stats_row, "Kullanılan", str(used), "#3B8ED0", 0)
            self.create_stat_box(stats_row, "Kalan", str(remaining), 
                               "#2CC985" if remaining > 0 else "#E04F5F", 1)
            self.create_stat_box(stats_row, "Toplam", str(limit), "#888888", 2)
        else:
            # Legacy or unlimited plan
            infinity_frame = ctk.CTkFrame(scroll_frame, fg_color="#3B8ED0", corner_radius=10)
            infinity_frame.pack(fill="x", pady=10)
            ctk.CTkLabel(infinity_frame, text="♾️ Sınırsız", 
                        font=("Roboto", 18, "bold"), 
                        text_color="white").pack(pady=15)
        
        # Payment Information
        self.create_info_section(scroll_frame, "💰 Ödeme Bilgileri", top_padding=20)
        ...
        self.create_info_row(scroll_frame, "Toplam Fiyat", f"{purchase_price:.2f} TL")
        self.create_info_row(scroll_frame, "Ödenen", f"{total_paid:.2f} TL", 
                            text_color="#2CC985" if total_paid > 0 else "gray")
        self.create_info_row(scroll_frame, "Kalan Borç", f"{remaining_debt:.2f} TL", 
                            text_color="#E04F5F" if remaining_debt > 0 else "#2CC985")
        
        # Payment History
        if payments:
            self.create_info_section(scroll_frame, f"📝 Ödeme Geçmişi ({len(payments)})", top_padding=20)
        ...
        ctk.CTkButton(btn_frame, text="QR Kod OKUT", width=150, height=40,
                     font=("Roboto", 14, "bold"),
                     fg_color="#2CC985",
                     command=self.simulate_qr_scan).pack(side="left", padx=5)
        
        ctk.CTkButton(btn_frame, text="Kapat", width=150, height=40,
                     font=("Roboto", 14, "bold"),
    ...
    def simulate_qr_scan(self):
        ...
        try:
            ...
            if not subscription_id:
                self.show_error("Abonelik ID'si bulunamadı")
                return
    ...
    def show_error(self, message: str):
        ...
        ctk.CTkLabel(error_window, text=f"❌ Hata\n\n{message}", 
                    font=("Roboto", 14),
                    text_color="#E04F5F",
                    justify="center").pack(pady=20, padx=20)
        
        ctk.CTkButton(error_window, text="Tamam", 
                     fg_color="gray",
    ...
    def show_success(self, title: str, message: str):
        ...
        ctk.CTkButton(success_window, text="Tamam", 
                     fg_color="#2CC985",

...
class AddMemberDialog(ctk.CTkToplevel):
    """Dialog for adding a new member with validation"""
    ...
    def save(self):
        ...
        first_name = self.entry_first_name.get().strip().title()
        last_name = self.entry_last_name.get().strip().title()
        ...
        try:
            self.api_client.post("/api/v1/members/", json=data)
            self.on_success()
            self.destroy()

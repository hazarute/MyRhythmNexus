from __future__ import annotations

import os
import re
import sys
from pathlib import Path
from typing import Optional


def read_version_from_config() -> Optional[str]:
    cfg = Path('desktop') / 'core' / 'config.py'
    if not cfg.exists():
        return None
    text = cfg.read_text(encoding='utf-8')
    m = re.search(r"DesktopConfig\.VERSION\s*=\s*['\"]([^'\"]+)['\"]", text)
    if m:
        return m.group(1).strip()
    try:
        repo_root = Path.cwd()
        if str(repo_root) not in sys.path:
            sys.path.insert(0, str(repo_root))
        from desktop.core.config import DesktopConfig  # type: ignore

        v = getattr(DesktopConfig, 'VERSION', None)
        if v:
            return str(v)
    except Exception:
        pass
    ver_file = Path('desktop') / 'version.txt'
    if ver_file.exists():
        try:
            return ver_file.read_text(encoding='utf-8').strip()
        except Exception:
            pass
    return None


def find_built_exe(version: Optional[str] = None, prefer_linux: bool = False) -> Optional[Path]:
    d = Path('dist')
    if d.exists():
        files = [f for f in d.iterdir() if f.is_file()]
        if not files:
            return None
        
        # Filter candidates based on preference
        if prefer_linux:
            # Prefer no extension or typical linux extensions
            candidates = [f for f in files if not f.suffix or f.suffix in ('.bin', '.sh')]
            if not candidates:
                candidates = files
        else:
            # Prefer .exe
            candidates = [f for f in files if f.suffix.lower() == '.exe']
            if not candidates:
                # Fallback to all files if no exe found (though unexpected for Windows build)
                candidates = files

        if version:
            ver_name = f'MyRhythmNexus_v{version}'
            # Exact match first
            for candidate in candidates:
                if candidate.name == ver_name or candidate.stem == ver_name:
                    return candidate
            # Then partial match
            for candidate in candidates:
                if ver_name in candidate.name:
                    return candidate
        
        # Fallback: any MyRhythmNexus file
        for candidate in candidates:
            if candidate.name.startswith('MyRhythmNexus'):
                return candidate
        
        return candidates[0] if candidates else None
    r = Path('release')
    if r.exists():
        exes = list(r.glob('*.exe'))
        if exes:
            for e in exes:
                if 'MyRhythmNexus' in e.name:
                    return e
            return exes[0]
    return None

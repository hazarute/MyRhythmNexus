...
class ProfileTab:
    ...
    def setup(self):
        ...
        try:
            ...
            self.create_stat_card(stats_frame, "Üyelik Durumu", status_text, "✅" if status_text=="Aktif" else "❌", "#26813A" if status_text=="Aktif" else "#808080", 3)

        except Exception as e:
            print(f"Error loading dashboard stats: {e}")
            ctk.CTkLabel(stats_frame, text="İstatistikler yüklenemedi").pack()
        ...
        ctk.CTkLabel(left_frame, text="📦 Aktif Paketler", font=("Roboto", 16, "bold")).pack(anchor="w", pady=(0, 10))
        ...
        if subs:
            ...
            if not active_subs:
                ctk.CTkLabel(packages_scroll, text="Aktif paket yok.", text_color="gray").pack(pady=10)
            else:
                for sub in active_subs:
                    self.create_profile_package_card(packages_scroll, sub)
        else:
            ctk.CTkLabel(packages_scroll, text="Paket bulunamadı.", text_color="gray").pack(pady=10)

        # Right: Recent Activity
        right_frame = ctk.CTkFrame(self.parent, fg_color="transparent")
        right_frame.grid(row=1, column=1, sticky="nsew", padx=10, pady=10)
        
        ctk.CTkLabel(right_frame, text="🕒 Son Hareketler (QR)", font=("Roboto", 16, "bold")).pack(anchor="w", pady=(0, 10))
        ...
        if checkins:
            ...
            ctk.CTkLabel(activity_scroll, text="Hareket yok.", text_color="gray").pack(pady=10)
    ...
    def create_profile_package_card(self, parent, sub):
        ...
        ctk.CTkLabel(content, text=f"📦 {pkg_name}", 
                    font=("Roboto", 18, "bold"), 
    ...
    def create_activity_item(self, parent, chk):
        ...
        if verified_by != 'Sistem':
            ctk.CTkLabel(content, text=f"✓ {verified_by}", font=("Roboto", 10), text_color="gray60", anchor="w").pack(fill="x")
        else:
            ctk.CTkLabel(content, text="🤖 Otomatik", font=("Roboto", 10), text_color="gray60", anchor="w").pack(fill="x")
